...
function makeSale(overrides: Partial<Sale> = {}): Sale {
  ...
  return {
    ...
    clientId: 'cli-1', vehicleId: 'veh-1', currencyId: 'cur-dop',
    salePrice: PRECIO_VENTA, exchangeRate: 1, saleDate: '2026-03-01',
...
describe('Notas de credito', () => {
  ...
  beforeEach(async () => {
    ...
    crear = new CreateCreditNoteUseCase(invoices);
  ...
  it('no se pueden emitir contra una factura que la DGII aun no acepto', async () => {
    ...
    const result = await crear.execute({
      invoiceId: pendiente.id, reason: 'x', amount: 1000, actorUserId: 'u',
    });
  ...
  it('rechaza una nota que supera el importe de la factura', async () => {
    const result = await crear.execute({
      invoiceId, reason: 'Devolucion total', amount: PRECIO_VENTA + 1, actorUserId: 'u',
    });
  ...
  it('cuenta las notas PENDIENTES al calcular lo disponible', async () => {
    await crear.execute({ invoiceId, reason: 'Parcial 1', amount: 1_000_000, actorUserId: 'u' });

    // Quedan 800.000 disponibles aunque la primera aun no se haya emitido.
    const result = await crear.execute({
      invoiceId, reason: 'Parcial 2', amount: 900_000, actorUserId: 'u',
    });
  ...
  it('exige que el NCF de la nota sea de tipo E34', async () => {
    const nota = await crear.execute({ invoiceId, reason: 'Devolucion', amount: 100_000, actorUserId: 'u' });
  ...
  it('una nota parcial reduce el neto pero NO anula la factura', async () => {
    const nota = await crear.execute({ invoiceId, reason: 'Descuento', amount: 300_000, actorUserId: 'u' });
    if (!nota.ok) throw new Error('no se pudo crear la nota');
  ...
  it('al cubrir el importe completo, ANULA la factura', async () => {
    const nota = await crear.execute({
      invoiceId, reason: 'Devolucion total del vehiculo', amount: PRECIO_VENTA, actorUserId: 'u',
    });
    if (!nota.ok) throw new Error('no se pudo crear la nota');
  ...
  it('varias notas parciales que suman el total tambien anulan la factura', async () => {
    for (const [monto, ncf] of [[900_000, 'E340000000010'], [900_000, 'E340000000011']] as const) {
      const nota = await crear.execute({ invoiceId, reason: 'Parcial', amount: monto, actorUserId: 'u' });
      if (!nota.ok) throw new Error('no se pudo crear la nota');
